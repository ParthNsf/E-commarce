...
const uploadFile = require("../Ultis/Cloudinary");
...
const addVarients = async (req, res) => {
    try {
        ...
        const filesRes = await Promise.all(req.files.map(v => uploadFile(v.path, "products_img")));
...
const updateVarients = async (req, res) => {
    try {
        ...
        if (req.files && req.files.length > 0) {
            const filesRes = await Promise.all(req.files.map(file => uploadFile(file.path, "products_img")));
